...
parser.add_argument('--dataset',  default='re_celeba', help='cifar10 | lsun | imagenet | folder | lfw ')
...
if opt.dataset in ['imagenet', 'folder', 'lfw', 're_celeba']:
    # folder dataset
...
def load_pic(path):
    ...
    pic = transform(pic)
    #pic = pic.repeat(1, 1, 1, 1)
    return pic

origin_mask = load_pic('designed_mask.png')
origin_mask = origin_mask.byte()
...
for epoch in range(resume_epoch, opt.niter):
    for i, data in enumerate(dataloader, 0):
        real_cpu, _ = data
        mask = origin_mask.expand_as(real_cpu)
        real_center_cpu = real_cpu.clone()
